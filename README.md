...
The distributable is written to `build/libs/instancednotinfinite-1.0.jar`.
...
5. Minecraft's structure APIs place that same retained `StructureStart`, or an exactly measured NBT `StructureTemplate`.
6. Surface-like automatic structures detect a front from ground-level doors, gates, or open boundary geometry. Underground/cave structures choose a deterministic-random walkable point inside their real authored piece boxes and a direction through the surrounding solid stone. The server cuts the configured bridge/path and arrival platform, places the player on that platform facing the chosen interior, and places an exact-instance return portal behind the arrival point. Non-underground advanced definitions retain their configured relative entry behavior.
7. Players are teleported with persistent return locations. Entering the UUID-bound destination portal returns them to that saved origin, with a cooldown preventing immediate portal bounce. Multiple assigned players can occupy the same run, while other runs remain separate worlds.
...
Terrain is generated deterministically and lazily inside the dungeon's finite instance envelope without invoking vanilla density terrain, surface rules, or carvers. Broad interpolated noise creates smooth height variation; the outer band tapers in thickness and uses a deterministic three-dimensional dither mask to dissolve horizontally and vertically into void instead of ending in intact columns. Surface and floating-island palettes follow common biome categories (grass/dirt, sand/sandstone, red sand/terracotta, mycelium, or stone). `SAFE` decoration runs only the selected biome's vegetation and top-layer stages, preserving features such as trees, plants, kelp, snow, and ice without the expensive terrain pipeline. The guaranteed padded structure area never receives holes, chunks outside the envelope remain void, and the world border matches the actual envelope.
...
World holograms and catalyst icons interpret the cached snapshot mesh. Exact-dungeon inventory and recipe-viewer icons are rendered lazily as size-normalized, isometric 2D sprites in natural block colors, rasterized once at the independently configured icon resolution, and held in a bounded LRU cache. The supplied artwork is used only as the flat 2D placeholder while those GUI sprites are not ready. Held, dropped, framed, and other in-world item contexts use the completed upright 3D miniature whenever one is ready, otherwise retaining the animated quarter-scale portal cube and its six-face particle field; unbound catalysts retain that portal effect because they have no singular miniature. A structure-pool catalyst keeps one stable animation slot per catalogue member, using the 2D placeholder for an unavailable GUI sprite and the matching resolved-color portal cube for an unavailable in-world miniature. Its in-world presentation shrinks, swaps at zero scale, expands, and rotates continuously at the configured `poolItemSwapIntervalSeconds`, while its GUI presentation crossfades across the same stable member slots. Adding a newly generated icon therefore no longer changes the animation's count or remaps its current index. Once a complete exact-dungeon snapshot has produced an icon, its palette-compressed structure geometry is cached below the client game directory under `cache/instancednotinfinite/miniatures/<world UUID>`. Rejoining the same world rebuilds the GUI icon from that cache without manifesting the dungeon again; a persistent identity synchronized from the save prevents equally named dungeons in unrelated worlds from sharing geometry. The supplied portal artwork is packaged byte-for-byte as both the mod-list logo and the 2D placeholder used whenever no generated GUI snapshot is available.
...
At catalogue rebuild time the resolver reads the structure type, allowed-biome `HolderSet`, terrain adaptation, generation step, and encoded authored start height. It uses the allowed biomes as the default pool and generically classifies Nether, End, ocean-surface, underwater, buried, encapsulated, stronghold, and underground structures. Structures in the `strongholds` generation step use `UNDERGROUND`, including Integrated Stronghold, so they receive the solid encasement and intentional underground entry route instead of a surface approach. Constant uniform absolute start heights, such as Integrated Stronghold's authored Y=15, are retained as structure metadata. An all-ocean structure placed during `top_layer_modification`, or authored with an absolute start above sea level, uses `OCEAN_SURFACE`; other all-ocean structures remain `UNDERWATER`. High-absolute ocean structures preserve Minecraft's authored sea level through any vertical fitting instead of inheriting the temporary fallback plan's surface, while projected top-layer structures continue following the generated waterline. Low-confidence cases safely default to `SURFACE`. It does not generate structures during startup.
...
- `FLOATING_ISLAND` and `END_LIKE` use a finite suspended island with the same smooth/dithered edge treatment.
...
| `/dungeon enter [dungeon]` | operator | Creates and enters. With no ID, performs weighted pool selection. |
...
- Manifestation creation is server-thread owned but split into resumable chunk-sized work under the configured global time budget. The older direct `/dungeon create` and `/dungeon enter` APIs remain synchronous for compatibility; unusually large worldgen starts through those legacy paths can still cause a visible tick spike.
- Exact catalyst thumbnails require that the client has received one complete snapshot at least once for that world. JEI/EMI exact-target entries begin with the supplied placeholder, update after that dungeon has been manifested, and then restore from the world-scoped client cache across reconnects; weighted-pool catalysts cannot know their dungeon before server selection and intentionally remain static.
